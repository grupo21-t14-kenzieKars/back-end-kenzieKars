...
const listCarsService = async (page: number = 1, perpage: number = 9): Promise<ICarPagination> => {
  ...
  if(endPage < totalCount){
    nextPage = `http://localhost:3000/car?page=${page + 1}&perpage=${perpage}`
  }

  let prevPage: string | null = null
  if(page> 1){
    prevPage = `http://localhost:3000/car?page=${page - 1}&perpage=${perpage}`
  }
